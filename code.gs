...
function getMissingReports() {
  ...
  const master = getLineMaster().map(l => l.line);
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (sheetDate.getTime() === today.getTime()) {
      submitted.push(values[i][2]); // Prod Line column
    }
  }

  return master.filter(line => !submitted.includes(line));
}
...
function buildMissingReport() {
  ...
  sheet.getRange(4,1).setValue("Missing Lines");

  if (missing.length === 0) {
    sheet.getRange(5,1).setValue("All lines submitted.");
  } else {
    const output = missing.map(line => [line]);
    sheet.getRange(5,1,output.length,1).setValues(output);
  }
